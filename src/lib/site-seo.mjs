...
function inferSeoFromPath(path, title = '') {
	if (path === '/') {
		return {
			...
			description: buildMetaDescription(
				'imóveis na planta em Balneário Camboriú',
			...
			keywords: buildKeywords('comprar imóvel na planta', 'empreendimentos Balneário Camboriú'),
		};
	}
	...
	if (path === '/about') {
		return {
			...
			description: buildMetaDescription(
				'Viver Catarina',
				'Portal especializado em imóveis na planta em Balneário Camboriú, com curadoria de lançamentos e atendimento por corretores credenciados',
			),
			keywords: buildKeywords('quem somos', 'portal imobiliário Balneário Camboriú'),
		};
	}
	...
	if (path === '/contact') {
		return {
			title: 'Contato',
			description: buildMetaDescription(
				'contato imobiliário Balneário Camboriú',
			...
			keywords: buildKeywords('contato imobiliário', 'corretor Balneário Camboriú'),
		};
	}

	if (path === '/blog' || /^\/blog\/pagina-\d+$/.test(path)) {
		const { pageNumber } = parsePagination(path);

		return {
			title: `Blog${paginationSuffix(pageNumber)}`,
			description: buildMetaDescription(
				'blog imobiliário Balneário Camboriú',
				'Artigos sobre mercado imobiliário, bairros e dicas para comprar imóvel na planta em Balneário Camboriú e Santa Catarina',
			),
			keywords: buildKeywords('blog imobiliário', 'mercado imobiliário Balneário Camboriú'),
		};
	}

	if (path.startsWith('/blog/')) {
		const keyword = title || 'imóveis na planta em Balneário Camboriú';
		...
		return {
			...
			description: buildMetaDescription(
				keyword,
				title
					? `${title}. Conteúdo do Viver Catarina sobre bairros, mercado imobiliário e dicas para comprar na planta em Balneário Camboriú`
					: 'Artigo do blog Viver Catarina sobre imóveis na planta, bairros e mercado imobiliário em Balneário Camboriú',
			),
			keywords: buildKeywords(title, 'blog imobiliário Balneário Camboriú'),
		};
	}
	...
	if (path === '/lancamentos' || path.startsWith('/lancamentos/')) {
			...
			return {
				...
				description: withPagination(
					buildMetaDescription(
						'lançamentos imobiliários Balneário Camboriú',
				...
				keywords: buildKeywords('lançamentos Balneário Camboriú', 'imóveis na planta'),
			};
		}
		...
		if (basePath === '/lancamentos/terrenos') {
			return {
				...
				description: withPagination(
					buildMetaDescription(
						'terrenos Balneário Camboriú',
						'Encontre terrenos e lotes na planta em Balneário Camboriú. Compare localização, metragem, preços e condições de compra no Viver Catarina',
					),
					pageNumber,
				),
				keywords: buildKeywords('terrenos Balneário Camboriú', 'lotes na planta'),
			};
		}
	...
	if (path === '/bairros' || path.startsWith('/bairros/')) {
		...
		if (basePath === '/bairros') {
			return {
				...
				keywords: buildKeywords('bairros Balneário Camboriú', 'imóveis por bairro'),
			};
		}
			...
			return {
				...
				description: withPagination(
					buildMetaDescription(
						...
						`Compare empreendimentos na faixa ${filterLabel.toLowerCase()}. Veja metragens, plantas e condições de pagamento em Balneário Camboriú`,
					),
					pageNumber,
				),
				keywords: buildKeywords(filterLabel, 'imóveis na planta Balneário Camboriú', 'preço imóvel'),
			};
		}
	}
	...
	if (path.startsWith('/imovel/')) {
		const keyword = title ? `${title} Balneário Camboriú` : 'imóveis na planta em Balneário Camboriú';
		...
		return {
			...
			description: buildMetaDescription(
				keyword,
				title
					? `${title} na planta em Balneário Camboriú. Veja plantas, preços, fotos, condições de pagamento e fale com corretor credenciado no Viver Catarina`
					: 'Empreendimento na planta em Balneário Camboriú com plantas, preços, condições de pagamento e atendimento de corretores credenciados no Viver Catarina',
			),
			keywords: buildKeywords(title, 'empreendimento na planta', 'apartamento Balneário Camboriú'),
		};
	}
	...
	if (path === '/404' || path === '/error') {
		return {
			...
			description: buildMetaDescription(
				'Viver Catarina',
				'A página solicitada não foi encontrada no portal Viver Catarina. Volte ao início e continue buscando imóveis na planta em Balneário Camboriú',
			),
			keywords: buildKeywords(),
			noindex: true,
		};
	...
	if (isDemoPath(path)) {
		...
		return {
			...
			description: buildMetaDescription(
				pageTitle,
				`${pageTitle} do portal Viver Catarina. Explore imóveis na planta, lançamentos e apartamentos em Balneário Camboriú com curadoria e atendimento especializado`,
			),
	...
	return {
		...
		description: buildMetaDescription(
			title || 'imóveis na planta em Balneário Camboriú',
			title
				? `${title} no portal Viver Catarina. Compare lançamentos, plantas, preços e condições para comprar imóvel na planta em Balneário Camboriú`
...
export function buildLancamentosListingSeo(label, pathname, pageNumber = 1) {
	...
	const keyword = `${label.toLowerCase()} na planta Balneário Camboriú`;
	...
	return {
		...
		keywords: buildKeywords(label, 'lançamentos Balneário Camboriú', 'comprar na planta'),
...
export function buildBairroListingSeo(neighborhoodName, pathname, pageNumber = 1, options = {}) {
	...
	return {
		...
		description,
		keywords: buildKeywords(neighborhoodName, 'imóveis na planta', 'bairro Balneário Camboriú'),
		canonicalPath: normalizePath(pathname),
	};
}
...
export function resolvePageSeo(options = {}) {
	...
	const resolvedDescription =
		description || inferred.description || buildMetaDescription('imóveis na planta em Balneário Camboriú', SITE_DEFAULT_DESCRIPTION);
