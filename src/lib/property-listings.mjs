...
import { filterBalnearioCamboriuListings, toWebpAssetPath } from './property-data.mjs';
import { buildPropertyImageTitle, buildPropertyListingImageAlt } from './property-image-alt.mjs';
import { cleanPropertyTitle } from './property-slug.mjs';
import { formatBrCurrency, formatPropertyPriceLabel } from './property-price.mjs';
...
export function formatPrice(amount) {
	return formatBrCurrency(amount) || 'Consulte';
}
...
export function getThumbnailUrl(thumbnail) {
	if (!thumbnail) {
		return '/assets/img/popular/popular-1-1.webp';
	}

	return `/data/${toWebpAssetPath(thumbnail)}`;
}
...
export function loadNeighborhoodListing(slug) {
	...
	const properties = filterBalnearioCamboriuListings(listing.properties || []);
...
export function listActiveNeighborhoods() {
	...
	return index.neighborhoods.sort((a, b) => a.name.localeCompare(b.name, 'pt-BR'));
}
...
export function enrichProperty(entry, options = {}) {
	...
	const priceAmount =
		property?.price?.amount != null
			...
			: entry.price > 0
				? entry.price
				: 0;
	const title = cleanPropertyTitle(entry.title, property);
	...
	return {
		...
		price: priceAmount,
		priceLabel: formatPropertyPriceLabel(property?.price, priceAmount),
		categoryLabel: getCategoryLabel(entry.category),
		detailUrl: getPropertyUrl(entry.slug),
		imageUrl: getThumbnailUrl(entry.thumbnail),
		imageAlt: property ? buildPropertyListingImageAlt(property) : title,
		imageTitle: property ? buildPropertyImageTitle(property) : title,
...
function buildFeaturedSidebarItem(property) {
	...
	return `<div class="recent-post">
                                            <div class="media-img">
                                                <a href="${detailUrl}"><img src="${imageUrl}" alt="${escapeHtml(property.imageAlt || title)}" title="${escapeHtml(property.imageTitle || title)}"></a>
                                            </div>
