...
import { BLOG_POSTS_PER_PAGE, getBlogListingPageCount } from './blog-listing-page.mjs';
import {
...
export function getPagesSitemapEntries() {
	...
	const blogPageCount = getBlogListingPageCount(getBlogPosts(), BLOG_POSTS_PER_PAGE);
	for (let page = 2; page <= blogPageCount; page += 1) {
		entries.push(
			buildSitemapEntry(`/blog/pagina-${page}`, { changefreq: 'weekly', priority: '0.7' }),
		);
	}
...
export function getBlogSitemapEntries() {
	return getBlogPosts().map((post) =>
		buildSitemapEntry(post.href, {
			changefreq: 'monthly',
			priority: '0.7',
			lastmod: post.dateUpdated ?? post.datePublished,
		}),
	);
}
...
> Portal especializado em lançamentos imobiliários e imóveis na planta em Balneário Camboriú, Santa Catarina.

O Viver Catarina conecta compradores a apartamentos, casas em condomínio e loteamentos em Balneário Camboriú. Cada empreendimento tem ficha técnica, plantas, preços e contato via WhatsApp.
...
- [Lançamentos](${SITE_URL}/lancamentos): listagem de imóveis novos em Balneário Camboriú
