/**
 * Reaplica linkagem do cluster Centro (pilar + satélites publicados).
 * Uso: node scripts/rebuild-centro-cluster-links.mjs
 */
...
import {
	...
	PILLAR,
	blogHref,
	rebuildCentroArticle,
} from '../src/lib/centro-cluster.mjs';
...
const publishedSlugs = [...CENTRO_CLUSTER_SLUGS].filter((slug) =>
...
for (const slug of publishedSlugs) {
	...
	const rebuilt = rebuildCentroArticle(raw, slug);
...
if (!process.exitCode) {
	console.log(`OK: ${publishedSlugs.length} artigo(s) do cluster Centro.`);
}
