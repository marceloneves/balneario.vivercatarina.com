...
import { encodeWebpFile } from '../src/lib/webp-process.mjs';
import { BALNEARIO_CAMBORIU_CITY_SLUG, sanitizeImportedLocation } from '../src/lib/property-data.mjs';
...
function toImportedWebpName(fileName) {
	const baseName = fileName.replace(RASTER_EXT, '');
	return `${baseName}.webp`;
}

async function writeImportedImageAsWebp(sourcePath, targetPath) {
	await encodeWebpFile(sourcePath, targetPath);
}

async function copyImages(imagePaths, htmlFile, imagesDir) {
	const copied = [];
	let index = 0;
	...
	for (const relativePath of imagePaths) {
		...
		const safeName = `${String(index).padStart(2, '0')}-${toImportedWebpName(originalName)}`;
		const targetPath = join(imagesDir, safeName);
		await writeImportedImageAsWebp(sourcePath, targetPath);
		copied.push({
...
async function main() {
	...
	for (const htmlFile of files) {
		...
		if (parsed.address.city?.slug !== BALNEARIO_CAMBORIU_CITY_SLUG) {
			skipped.push(`${sourceSlug} (fora de Balneário Camboriú)`);
			continue;
		}
		...
		const copiedImages = await copyImages(parsed.imagePaths, htmlFile, imagesDir);
