/**
 * Reaplica linkagem do cluster Barra Sul (pilar + satélites publicados).
 * Uso: node scripts/rebuild-barra-sul-cluster-links.mjs
 */
...
import {
	...
	PILLAR,
	blogHref,
	rebuildBarraSulArticle,
} from '../src/lib/barra-sul-cluster.mjs';
...
const publishedSlugs = [...BARRA_SUL_CLUSTER_SLUGS].filter((slug) =>
...
for (const slug of publishedSlugs) {
	...
	const rebuilt = rebuildBarraSulArticle(raw, slug);
...
if (!process.exitCode) {
	console.log(`OK: ${publishedSlugs.length} artigo(s) do cluster Barra Sul.`);
}
