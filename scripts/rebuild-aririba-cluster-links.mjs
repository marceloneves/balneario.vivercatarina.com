/**
 * Reaplica linkagem do cluster Ariribá (pilar + satélites publicados).
 * Uso: node scripts/rebuild-aririba-cluster-links.mjs
 */
...
import {
	...
	PILLAR,
	blogHref,
	rebuildAriribaArticle,
} from '../src/lib/aririba-cluster.mjs';
...
const publishedSlugs = [...ARIRIBA_CLUSTER_SLUGS].filter((slug) =>
...
for (const slug of publishedSlugs) {
	...
	const rebuilt = rebuildAriribaArticle(raw, slug);
...
if (!process.exitCode) {
	console.log(`OK: ${publishedSlugs.length} artigo(s) do cluster Ariribá.`);
}
