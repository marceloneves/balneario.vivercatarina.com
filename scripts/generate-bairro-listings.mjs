...
const neighborhoodsPath = join(root, 'src/data/balneario-camboriu-neighborhoods.json');
...
function main() {
	...
	for (const entry of imoveisIndex.properties) {
		if (entry.city !== BALNEARIO_CAMBORIU_CITY_SLUG) {
			continue;
		}
	...
	for (const neighborhood of neighborhoods) {
		...
		const payload = {
			...
			city: {
				slug: BALNEARIO_CAMBORIU_CITY_SLUG,
	...
	writeFileSync(
		...
		`${JSON.stringify(
			{
				city: BALNEARIO_CAMBORIU_CITY_SLUG,
				generatedAt: new Date().toISOString(),
	...
	console.log(`Imóveis em Balneário Camboriú: ${summary.reduce((acc, item) => acc + item.total, 0)}`);
