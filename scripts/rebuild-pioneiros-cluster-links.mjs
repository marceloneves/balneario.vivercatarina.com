/**
 * Reaplica linkagem do cluster Pioneiros (pilar + satélites publicados).
 * Uso: node scripts/rebuild-pioneiros-cluster-links.mjs
 */
...
import {
	...
	PILLAR,
	blogHref,
	rebuildPioneirosArticle,
} from '../src/lib/pioneiros-cluster.mjs';
...
const publishedSlugs = [...PIONEIROS_CLUSTER_SLUGS].filter((slug) =>
...
for (const slug of publishedSlugs) {
	...
	const rebuilt = rebuildPioneirosArticle(raw, slug);
...
if (!process.exitCode) {
	console.log(`OK: ${publishedSlugs.length} artigo(s) do cluster Pioneiros.`);
}
