/**
 * Reaplica linkagem do cluster Barra (pilar + satélites publicados).
 * Uso: node scripts/rebuild-barra-cluster-links.mjs
 */
...
import {
	...
	blogHref,
	rebuildBarraArticle,
} from '../src/lib/barra-cluster.mjs';
...
const publishedSlugs = [...BARRA_CLUSTER_SLUGS].filter((slug) =>
...
for (const slug of publishedSlugs) {
	...
	const rebuilt = rebuildBarraArticle(raw, slug);
...
if (!process.exitCode) {
	console.log(`OK: ${publishedSlugs.length} artigo(s) do cluster Barra.`);
}
