/**
 * Reaplica linkagem do cluster Construtoras e Incorporadoras.
 * Uso: node scripts/rebuild-construtoras-cluster-links.mjs
 */
...
import {
	...
	rebuildConstrutorasArticle,
} from '../src/lib/construtoras-cluster.mjs';
...
const publishedClusterSlugs = [...CONSTRUTORAS_CLUSTER_SLUGS].filter((slug) => {
	try {
...
for (const slug of publishedClusterSlugs) {
	...
	const rebuilt = rebuildConstrutorasArticle(raw, slug);
...
if (!process.exitCode) {
	console.log('OK: cluster Construtoras e Incorporadoras');
}
