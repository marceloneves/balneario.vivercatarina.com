/**
 * Reaplica linkagem do cluster Nações (pilar + satélites publicados).
 * Uso: node scripts/rebuild-nacoes-cluster-links.mjs
 */
...
import {
	...
	PILLAR,
	blogHref,
	rebuildNacoesArticle,
} from '../src/lib/nacoes-cluster.mjs';
...
const publishedSlugs = [...NACOES_CLUSTER_SLUGS].filter((slug) =>
...
for (const slug of publishedSlugs) {
	...
	const rebuilt = rebuildNacoesArticle(raw, slug);
...
if (!process.exitCode) {
	console.log(`OK: ${publishedSlugs.length} artigo(s) do cluster Nações.`);
}
